...
class RegularisedMarkowitzExperiment:
    ...
    def portfolio_variance(self, weights):
        return np.dot(weights.T, np.dot(self.covariance, weights)) + self.regularisation*(np.square(np.sum(np.sqrt(np.abs(weights)))))
...
class RegularisedMarkowitzExperimentRiskAppetite:
    ...
    def __init__(self, returns, covariance, regularisation=0, risk_appetite=0.5):
        self.returns = returns
        self.covariance = covariance
        self.n = covariance.shape[0]
        self.target_return = None
    ...
    def optimize_portfolio(self, initial_weights):
        constraints = ({'type': 'ineq', 'fun': lambda x: self.portfolio_return(x) - self.target_return},
                       {'type': 'eq', 'fun': lambda x: np.sum(x) - 1})
        bounds = tuple((0, self.risk_appetite) for asset in range(self.n))
        result = minimize(self.portfolio_variance, initial_weights, method='SLSQP', bounds=bounds, constraints=constraints)
        return result.x
